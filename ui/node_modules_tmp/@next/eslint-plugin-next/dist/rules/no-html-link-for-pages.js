...
module.exports = (0, _definerule.defineRule)({
   ...
   */ create: function create(context) {
    ...
    var _ruleOptions = _sliced_to_array(ruleOptions, 1),
      customPagesDirectory = _ruleOptions[0];
    var rootDirs = (0, _getrootdirs.getRootDirs)(context);
    var pagesDirs = (
      customPagesDirectory
        ? [customPagesDirectory]
        : rootDirs.map(function (dir) {
            return [_path.join(dir, "pages"), _path.join(dir, "src", "pages")];
          })
    ).flat();
    ...
    var appDirs = rootDirs
      .map(function (dir) {
        return [_path.join(dir, "app"), _path.join(dir, "src", "app")];
      })
      .flat();
          ...
          if (pageUrl.test((0, _url.normalizeURL)(hrefPath))) {
            context.report({
              ...
              message: "Do not use an `<a>` element to navigate to `"
                .concat(
                  hrefPath,
                  "`. Use `<Link />` from `next/link` instead. See: ",
                )
                .concat(url),
            });
          }
