 ...
 */ function parseUrlForPages(urlprefix, directory) {
  ...
  var _;
  (_ = (_fsReadDirSyncCache = fsReadDirSyncCache)[(_directory = directory)]) !==
    null && _ !== void 0
    ? _
    : (_fsReadDirSyncCache[_directory] = _fs.readdirSync(directory, {
        withFileTypes: true,
      }));
  var res = [];
    ...
    if (/(\.(j|t)sx?)$/.test(dirent.name)) {
      if (/^index(\.(j|t)sx?)$/.test(dirent.name)) {
        res.push(
          ""
            .concat(urlprefix)
            .concat(dirent.name.replace(/^index(\.(j|t)sx?)$/, "")),
        );
      }
      res.push(
        "".concat(urlprefix).concat(dirent.name.replace(/(\.(j|t)sx?)$/, "")),
      );
    } else {
      ...
      if (dirent.isDirectory() && !dirent.isSymbolicLink()) {
        ...
        (_res = res).push.apply(
          ...
          _to_consumable_array(
            parseUrlForPages(urlprefix + dirent.name + "/", dirPath),
          ),
        );
      }
    }
