...
export class GridKeyboardDelegate<T, C extends GridCollection<T>>
  ...
  getKeyBelow(fromKey: Key) {
    ...
    if (key != null) {
      ...
      if (this.isCell(startItem)) {
        ...
        return (
          getNthItem(getChildNodes(item, this.collection), startItem.index ?? 0)
            ?.key ?? null
        );
      }
  ...
  getFirstKey(fromKey?: Key, global?: boolean) {
    ...
    if (key != null) {
      ...
      if (this.isCell(item) && !global && item.parentKey != null) {
        ...
        return (
          getFirstItem(getChildNodes(parent, this.collection))?.key ?? null
        );
      }
    }
      ...
      if (item.textValue) {
        ...
        if (this.collator.compare(substring, search) === 0) {
          if (this.isRow(item) && this.focusMode === "cell") {
            return (
              getFirstItem(getChildNodes(item, this.collection))?.key ?? null
            );
          }
