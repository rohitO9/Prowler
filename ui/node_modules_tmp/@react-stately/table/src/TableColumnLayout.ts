...
export class TableColumnLayout<T> {
  ...
  resizeColumnWidth(
    ...
    controlledWidths: Map<Key, ColumnSize>,
    uncontrolledWidths: Map<Key, ColumnSize>,
    col = null,
    width: number,
  ): Map<Key, ColumnSize> {
    ...
    let resizingChanged = new Map<Key, ColumnSize>([
      ...controlledWidths,
      ...uncontrolledWidths,
    ]);
    ...
    collection.columns.forEach((column, i) => {
      ...
      if (
        col !== column.key &&
        !column.props.width &&
        !isStatic(uncontrolledWidths.get(column.key))
      ) {
        // uncontrolled don't have props.width for us, so instead get from our state
        frKey = column.key;
        frValue = parseFractionalUnit(
          uncontrolledWidths.get(column.key) as string,
        );
      } else if (
        col !== column.key &&
        !isStatic(column.props.width) &&
  ...
  buildColumnWidths(
    ...
    columnWidths.forEach((width, index) => {
      ...
      this.columnMinWidths.set(
        key,
        getMinWidth(
          column.props.minWidth ?? this.getDefaultMinWidth(column),
          tableWidth,
        ),
      );
      this.columnMaxWidths.set(
        key,
        getMaxWidth(column.props.maxWidth, tableWidth),
      );
    });
