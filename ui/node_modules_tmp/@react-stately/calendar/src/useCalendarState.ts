...
export function useCalendarState<T extends DateValue = DateValue>(
  ...
  let defaultFormatter = useMemo(
    () => new DateFormatter(props.locale),
    [props.locale],
  );
  let resolvedOptions = useMemo(
  ...
  let [lastCalendarIdentifier, setLastCalendarIdentifier] = useState(
    calendar.identifier,
  );
  ...
  function setValue(newValue: CalendarDate | null) {
    if (!props.isDisabled && !props.isReadOnly) {
      ...
      newValue = toCalendar(
        newValue,
        value?.calendar || new GregorianCalendar(),
      );
  ...
  return {
    ...
    focusNextPage() {
      ...
      setFocusedDate(
        constrainValue(focusedDate.add(pageDuration), minValue, maxValue),
      );
      setStartDate(
        alignStart(
          constrainStart(
    ...
    focusPreviousPage() {
      ...
      setFocusedDate(
        constrainValue(focusedDate.subtract(pageDuration), minValue, maxValue),
      );
      setStartDate(
        alignStart(
          constrainStart(
    ...
    isPreviousVisibleRangeInvalid() {
      ...
      return (
        isSameDay(prev, startDate) || this.isInvalid(prev, minValue, maxValue)
      );
    },
    isNextVisibleRangeInvalid() {
      ...
      return (
        isSameDay(next, endDate) || this.isInvalid(next, minValue, maxValue)
      );
    },
