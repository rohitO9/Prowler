...
async function renderToHTMLImpl(req, res, pathname, query, renderOpts, extra) {
    ...
    const { err, dev = false, ampPath = "", pageConfig = {}, buildManifest, reactLoadableManifest, ErrorDebug, getStaticProps, getStaticPaths, getServerSideProps, isNextDataRequest, params, previewProps, basePath, images, runtime: globalRuntime, isExperimentalCompile, swrDelta } = renderOpts;
    const { App } = extra;
    ...
    const content = await (0, _nodewebstreamshelper.streamToString)((0, _nodewebstreamshelper.chainStreams)((0, _nodewebstreamshelper.streamFromString)(prefix), await documentResult.bodyResult(renderTargetSuffix)));
