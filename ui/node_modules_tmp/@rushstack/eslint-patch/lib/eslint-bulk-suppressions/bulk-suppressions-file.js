...
exports.serializeSuppression =
  exports.deleteBulkSuppressionsFileInEslintrcFolder =
  exports.writeSuppressionsJsonToFile =
  exports.getAllBulkSuppressionsConfigsByEslintrcFolderPath =
  exports.getSuppressionsConfigForEslintrcFolderPath =
    void 0;
const fs_1 = __importDefault(require("fs"));
const constants_1 = require("./constants");
const IS_RUNNING_IN_VSCODE =
  process.env[constants_1.VSCODE_PID_ENV_VAR_NAME] !== undefined;
...
function getSuppressionsConfigForEslintrcFolderPath(eslintrcFolderPath) {
  const cachedSuppressionsConfig =
  ...
  if (cachedSuppressionsConfig) {
    shouldLoad =
      ...
      cachedSuppressionsConfig.readTime < Date.now() - TEN_SECONDS_MS;
...
function writeSuppressionsJsonToFile(eslintrcFolderPath, suppressionsConfig) {
  ...
  if (suppressionsConfig.jsonObject.suppressions.length === 0) {
    ...
    fs_1.default.writeFileSync(
      suppressionsPath,
      JSON.stringify(suppressionsConfig.jsonObject, undefined, 2),
    );
  }
}
...
function validateSuppressionsJson(json) {
  ...
  const EXPECTED_SUPPRESSION_PROPERTY_NAMES = new Set([
    "file",
    "scopeId",
    "rule",
  ]);
  for (const suppression of suppressions) {
    if (typeof suppression !== "object") {
      throw new Error(
        `Invalid suppression: ${JSON.stringify(suppression, null, 2)}`,
      );
    }
    if (!suppression) {
      throw new Error(
        `Suppression is null: ${JSON.stringify(suppression, null, 2)}`,
      );
    }
