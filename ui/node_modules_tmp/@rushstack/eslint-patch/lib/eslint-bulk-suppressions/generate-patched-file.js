...
function generatePatchedLinterJsFileIfDoesNotExist(
  ...
  const generateEnvVarValue =
    process.env[constants_1.ESLINT_BULK_FORCE_REGENERATE_PATCH_ENV_VAR_NAME];
  if (
    generateEnvVarValue !== "true" &&
    generateEnvVarValue !== "1" &&
    fs_1.default.existsSync(outputFilePath)
  ) {
    return;
  }
  ...
  function scanUntilMarker(marker) {
    ...
    throw new Error(
      "Unexpected end of input while looking for " + JSON.stringify(marker),
    );
  }
  ...
  function getIndexOfNextPublicMethod(fromIndex) {
    ...
    const afterMarkerIndex =
      rest.indexOf(markerForStartOfClassMethod) +
      markerForStartOfClassMethod.length;
    const isPublicMethod =
      rest[afterMarkerIndex] !== "_" &&
      rest[afterMarkerIndex] !== "#" &&
      !rest
        .substring(afterMarkerIndex, rest.indexOf("\n", afterMarkerIndex))
        .includes("static") &&
      !rest
        .substring(afterMarkerIndex, rest.indexOf("\n", afterMarkerIndex))
...
`;
  ...
  //
  outputFile += requireSection.replace(
    /require\s*\((?:'([^']+)'|"([^"]+)")\)/g,
    (match, p1, p2) => {
      var _a;
      const importPath =
        (_a = p1 !== null && p1 !== void 0 ? p1 : p2) !== null && _a !== void 0
          ? _a
          : "";
      if (importPath !== "path") {
        if (p1) {
...
exports.generatePatchedLinterJsFileIfDoesNotExist =
  generatePatchedLinterJsFileIfDoesNotExist;
