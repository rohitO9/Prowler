...
function getNameFromIndexSignature(node) {
  const propName = node.parameters.find(
    (parameter) => parameter.type === utils_1.AST_NODE_TYPES.Identifier,
  );
...
function typeNodeRequiresParentheses(node, text) {
  return (
    ...
    (node.type === utils_1.AST_NODE_TYPES.TSUnionType &&
      text.startsWith("|")) ||
...
function isParenlessArrowFunction(node, sourceCode) {
  return (
    ...
    !(0, astUtils_1.isParenthesized)(node.params[0], sourceCode)
  );
}
