...
function getMemberHeadLoc(sourceCode, node) {
  ...
  if (node.decorators.length === 0) {
    ...
    const nextToken = (0, eslint_utils_1.nullThrows)(
      sourceCode.getTokenAfter(lastDecorator),
      eslint_utils_1.NullThrowsReasons.MissingToken("token", "last decorator"),
    );
  ...
  if (!node.computed) {
    ...
    const closingBracket = (0, eslint_utils_1.nullThrows)(
      sourceCode.getTokenAfter(node.key, (token) => token.value === "]"),
      eslint_utils_1.NullThrowsReasons.MissingToken("]", node.type),
    );
