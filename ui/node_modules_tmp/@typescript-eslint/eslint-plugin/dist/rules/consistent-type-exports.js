...
function* fixExportInsertType(fixer, sourceCode, node) {
  const exportToken = (0, util_1.nullThrows)(
    sourceCode.getFirstToken(node),
    util_1.NullThrowsReasons.MissingToken("export", node.type),
  );
  yield fixer.insertTextAfter(exportToken, " type");
  for (const specifier of node.specifiers) {
    if (specifier.exportKind === "type") {
      const kindToken = (0, util_1.nullThrows)(
        sourceCode.getFirstToken(specifier),
        util_1.NullThrowsReasons.MissingToken("export", specifier.type),
      );
      const firstTokenAfter = (0, util_1.nullThrows)(
...
function* fixSeparateNamedExports(fixer, sourceCode, report) {
  ...
  const filteredSpecifierNames = valueSpecifiers
    ...
    .join(", ");
  const openToken = (0, util_1.nullThrows)(
    sourceCode.getFirstToken(node, util_1.isOpeningBraceToken),
    util_1.NullThrowsReasons.MissingToken("{", node.type),
  );
  const closeToken = (0, util_1.nullThrows)(
    sourceCode.getLastToken(node, util_1.isClosingBraceToken),
    util_1.NullThrowsReasons.MissingToken("}", node.type),
  );
