  ...
  create(contextWithoutDefaults) {
    ...
    function isDestructured(id) {
      return (
        ...
        (id.parent.type === utils_1.AST_NODE_TYPES.Property &&
          id.parent.shorthand) ||
        // `const { x = 2 }`
          ...
          handler: (node, validator) => {
            node.params.forEach((param) => {
              ...
              identifiers.forEach((i) => {
                ...
                if (isDestructured(i)) {
                  modifiers.add(
                    naming_convention_utils_1.Modifiers.destructured,
                  );
                }
      ...
      ':not(ObjectPattern) > Property[computed = false][kind = "init"][value.type != "ArrowFunctionExpression"][value.type != "FunctionExpression"][value.type != "TSEmptyBodyFunctionExpression"]':
        {
          ...
          handler: (node, validator) => {
            const modifiers = new Set([
              naming_convention_utils_1.Modifiers.public,
            ]);
...
function getIdentifiersFromPattern(pattern) {
  const identifiers = [];
  const visitor = new scope_manager_1.PatternVisitor({}, pattern, (id) =>
    identifiers.push(id),
  );
...
function isGlobal(scope) {
  ...
  return (
    scope.type === utils_1.TSESLint.Scope.ScopeType.global ||
