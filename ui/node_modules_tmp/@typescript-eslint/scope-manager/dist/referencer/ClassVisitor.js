...
class ClassVisitor extends Visitor_1.Visitor {
  ...
  visitClass(node) {
    ...
    __classPrivateFieldGet(
      ...
      "f",
    ).scopeManager.nestClassScope(node);
    if (node.id) {
  ...
  visitMethodFunction(node, methodNode) {
    if (node.id) {
      ...
      __classPrivateFieldGet(
        ...
        _ClassVisitor_referencer,
        "f",
      ).scopeManager.nestFunctionExpressionNameScope(node);
    }
    ...
    __classPrivateFieldGet(
      ...
      _ClassVisitor_referencer,
      "f",
    ).scopeManager.nestFunctionScope(node, true);
    /**
     * class A {
     *   @meta     // <--- check this
     *   foo(a: Type) {}
    ...
    this.visitType(node.typeParameters);
    __classPrivateFieldGet(this, _ClassVisitor_referencer, "f").visitChildren(
      node.body,
    );
    __classPrivateFieldGet(this, _ClassVisitor_referencer, "f").close(node);
  }
  visitPropertyBase(node) {
    ...
    if (node.value) {
      if (
        node.type === types_1.AST_NODE_TYPES.PropertyDefinition ||
        ...
        __classPrivateFieldGet(
          ...
          _ClassVisitor_referencer,
          "f",
        ).scopeManager.nestClassFieldInitializerScope(node.value);
      }
      __classPrivateFieldGet(this, _ClassVisitor_referencer, "f").visit(
        node.value,
      );
  ...
  StaticBlock(node) {
    __classPrivateFieldGet(
      ...
      _ClassVisitor_referencer,
      "f",
    ).scopeManager.nestClassStaticBlockScope(node);
...
function getLiteralMethodKeyName(node) {
  if (node.computed && node.key.type === types_1.AST_NODE_TYPES.Literal) {
    ...
    !node.computed &&
    node.key.type === types_1.AST_NODE_TYPES.Identifier
  ) {
