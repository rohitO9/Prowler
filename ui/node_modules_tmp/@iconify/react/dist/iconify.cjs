...
function quicklyValidateIconSet(obj) {
  ...
  for (const name in aliases) {
    ...
    if (
      ...
      (!icons[parent] && !aliases[parent]) || // Check other props
      !checkOptionalProps(icon, defaultExtendedIconProps)
    ) {
      return null;
    }
...
function listToIcons(list, validate = true, simpleNames = false) {
  ...
  list.forEach((item) => {
    const icon =
      typeof item === "string"
        ? stringToIcon(item, validate, simpleNames)
        : item;
    if (icon) {
      result.push(icon);
    }
...
const loadIcons = (icons, callback) => {
  ...
  sortedIcons.pending.forEach((icon) => {
    ...
    const storage = getStorage(provider, prefix);
    const pendingQueue =
      storage.pendingIcons ||
      (storage.pendingIcons = /* @__PURE__ */ new Set());
 ...
 * @param props - Component properties
 */
const InlineIcon = react.forwardRef((props, ref) =>
  IconComponent({
    inline: true,
    ...props,
    _ref: ref,
