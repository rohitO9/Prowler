"use client";
import { __esm, __export, __toCommonJS } from "./chunk-QEEKTJZK.mjs";
...
function createUseRender(forwardMotionProps = false) {
  const useRender = (Component2, props, ref, { latestValues }, isStatic) => {
    ...
    const renderedChildren = useMemo4(
...
function useForceUpdate() {
  ...
  const deferredForceRender = useCallback2(
...
var AnimatePresence = ({
  ...
  const forceRender =
    useContext5(LayoutGroupContext).forceRender || useForceUpdate()[0];
  const isMounted = useIsMounted();
...
function getKeyframes(value, valueName, target, transition) {
  ...
  const defaultOrigin =
    transition.from !== void 0 ? transition.from : value.get();
...
export { m, AnimatePresence, MotionConfig, LazyMotion, domAnimation };
