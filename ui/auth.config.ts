...
if (!apiBaseUrl) {
  throw new Error("apiBaseUrl is not defined. Check your environment variables.");
}
...
//   },
//   pages: {
//     signIn: "/sign-in",    // Path for sign-in page
//     newUser: "/sign-up",   // Path for new user sign-up
//   },
//   secret: process.env.NEXTAUTH_SECRET,  // Add secret from environment variable
...
//         return false; // Redirect users who are not logged in to the login page
//       } else if (isLoggedIn) {
//         return Response.redirect(new URL("/", nextUrl)); // Redirect to home if logged in
//       }
//       return true;
//     },
...
//     jwt: async ({ token, account, user }) => {
//       if (token?.accessToken) {
//         const decodedToken = jwtDecode(token.accessToken as string) as CustomJwtPayload;
//         if (typeof decodedToken.exp === "number") {
//           token.accessTokenExpires = decodedToken.exp * 1000; // Set token expiration time
//         } else {
//           token.accessTokenExpires = Date.now() + 60 * 60 * 1000; // fallback: 1 hour from now
//         }
//         token.user_id = decodedToken.user_id;
//         token.tenant_id = decodedToken.tenant_id;
//       }
...
//           tenantId: token.tenant_id,
//           accessToken: (user as User & { accessToken: JwtPayload }).accessToken,
//           refreshToken: (user as User & { refreshToken: JwtPayload }).refreshToken,
//           user: userInfo,
//         };
//       }
