...
export function FocusScope(props: FocusScopeProps) {
  ...
  useEffect(() => {
    const activeElement = getOwnerDocument(
    ...
    if (isElementInScope(activeElement, scopeRef.current)) {
      ...
      for (let node of focusScopeTree.traverse()) {
        if (
          node.scopeRef &&
          isElementInScope(activeElement, node.scopeRef.current)
        ) {
          scope = node;
        }
      }
  ...
  useLayoutEffect(() => {
    return () => {
      ...
      let parentScope =
        focusScopeTree.getTreeNode(scopeRef)?.parent?.scopeRef ?? null;
...
function createFocusManagerForScope(
  ...
  useLayoutEffect(() => {
    ...
    let onFocus = (e) => {
      ...
      if (
        ...
        isElementInScope(e.target, scopeRef.current)
      ) {
        activeScope = scopeRef;
        focusedNode.current = e.target;
      } else if (
        shouldContainFocus(scopeRef) &&
        !isElementInChildScope(e.target, scopeRef)
      ) {
    ...
    let onBlur = (e) => {
      ...
      raf.current = requestAnimationFrame(() => {
        ...
        if (
          ...
          shouldContainFocus(scopeRef) &&
          !isElementInChildScope(ownerDocument.activeElement, scopeRef)
        ) {
          activeScope = scopeRef;
...
function isElementInChildScope(element: Element, scope: ScopeRef = null) {
  ...
  for (let { scopeRef: s } of focusScopeTree.traverse(
...
function useAutoFocus(
  ...
  useEffect(() => {
    if (autoFocusRef.current) {
      ...
      if (
        !isElementInScope(ownerDocument.activeElement, activeScope.current) &&
        scopeRef.current
      ) {
...
function useRestoreFocus(
  ...
  useLayoutEffect(() => {
    ...
    let onKeyDown = (e: KeyboardEvent) => {
      ...
      if (
        !isElementInChildScope(focusedElement, scopeRef) ||
        !shouldRestoreFocus(scopeRef)
      ) {
        return;
      }
      ...
      if (
        (!nextElement || !isElementInChildScope(nextElement, scopeRef)) &&
        nodeToRestore
      ) {
...
function restoreFocusToElement(node: FocusableElement) {
  ...
  if (
    node.dispatchEvent(
      new CustomEvent(RESTORE_FOCUS_EVENT, { bubbles: true, cancelable: true }),
    )
  ) {
    focusElement(node);
  }
