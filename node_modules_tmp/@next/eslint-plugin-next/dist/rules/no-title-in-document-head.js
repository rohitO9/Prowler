    ...
    return {
      ImportDeclaration: function ImportDeclaration(node) {
        if (node.source.value === "next/document") {
          if (
            node.specifiers.some(function (param) {
      ...
      JSXElement: function JSXElement(node) {
        ...
        if (titleTag) {
          context.report({
            node: titleTag,
            message:
              "Do not use `<title>` element with `<Head />` component from `next/document`. Titles should defined at the page-level using `<Head />` from `next/head` instead. See: ".concat(
                url,
              ),
          });
        }
