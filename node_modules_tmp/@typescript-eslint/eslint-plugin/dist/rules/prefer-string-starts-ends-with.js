  ...
  create(context, [{ allowSingleElementEquality }]) {
    ...
    function isLengthExpression(node, expectedObjectNode) {
      if (node.type === utils_1.AST_NODE_TYPES.MemberExpression) {
        return (
          (0, util_1.getPropertyName)(node, globalScope) === "length" &&
    ...
    function getPropertyRange(node) {
      const dotOrOpenBracket = (0, util_1.nullThrows)(
        context.sourceCode.getTokenAfter(
          node.object,
          util_1.isNotClosingParenToken,
        ),
        util_1.NullThrowsReasons.MissingToken("closing parenthesis", "member"),
      );
    ...
    function getParent(node) {
      return (0, util_1.nullThrows)(
        node.parent?.type === utils_1.AST_NODE_TYPES.ChainExpression
          ? node.parent.parent
          : node.parent,
        ...
        context.report({
          ...
          *fix(fixer) {
            ...
            yield fixer.replaceTextRange(
              ...
              `${node.optional ? "?." : "."}${isStartsWith ? "start" : "end"}sWith`,
            );
          ...
          fix(fixer) {
            ...
            if (isStartsWith) {
              ...
              const posNodeIsAbsolutelyValid =
                (posNode.type === utils_1.AST_NODE_TYPES.BinaryExpression &&
                  posNode.operator === "-" &&
      ...
      'CallExpression > MemberExpression.callee[property.name="test"][computed=false]'(
        ...
        context.report({
          ...
          *fix(fixer) {
            ...
            const needsParen =
              argNode.type !== utils_1.AST_NODE_TYPES.Literal &&
            ...
            yield fixer.insertTextAfter(
              argNode,
              `${node.optional ? "?." : "."}${methodName}(${JSON.stringify(text)}`,
            );
          },
        });
      },
