...
function includesType(parserServices, node, typeFlagIn) {
  ...
  const types = (0, ts_api_utils_1.unionTypeParts)(
  ...
  switch (operand.comparisonType) {
    ...
    case "NotStrictEqualNull" /* NullishComparisonType.NotStrictEqualNull */: {
      ...
      if (
        ...
        (0, compareNodes_1.compareNodes)(
          operand.comparedName,
          nextOperand.comparedName,
...
function getFixer(sourceCode, parserServices, operator, options, chain) {
  ...
  if (
    ...
    if (
      ...
      (operator === "||" &&
        lastOperand.comparisonType ===
          "NotBoolean") /* NullishComparisonType.NotBoolean */
    ) {
  ...
  let newCode = parts
    .map((part) => {
      ...
      if (
        part.precedence !== util_1.OperatorPrecedence.Invalid &&
  ...
  if (lastOperand.node.type === utils_1.AST_NODE_TYPES.BinaryExpression) {
    ...
    const { left, right } = (() => {
      if (lastOperand.isYoda) {
        const unaryOperator =
          lastOperand.node.right.type === utils_1.AST_NODE_TYPES.UnaryExpression
      ...
      const unaryOperator =
        lastOperand.node.left.type === utils_1.AST_NODE_TYPES.UnaryExpression
  ...
  function flattenChainExpression(sourceCode, node) {
    switch (node.type) {
      ...
      case utils_1.AST_NODE_TYPES.CallExpression: {
        const argumentsText = (() => {
          const closingParenToken = (0, util_1.nullThrows)(
            sourceCode.getLastToken(node),
            util_1.NullThrowsReasons.MissingToken(
              "closing parenthesis",
              node.type,
            ),
          );
          const openingParenToken = (0, util_1.nullThrows)(
      ...
      case utils_1.AST_NODE_TYPES.MemberExpression: {
        ...
        return [
          ...
          {
            nonNull:
              node.object.type === utils_1.AST_NODE_TYPES.TSNonNullExpression,
            optional: node.optional,
...
function analyzeChain(context, parserServices, options, operator, chain) {
  ...
  const maybeReportThenReset = (newChainSeed) => {
    if (subChain.length > 1) {
      ...
      (0, checkNullishAndReport_1.checkNullishAndReport)(
        context,
        parserServices,
        options,
  ...
  for (let i = 0; i < chain.length; i += 1) {
    ...
    if (lastOperand) {
      ...
      if (comparisonResult === "Subset" /* NodeComparisonResult.Subset */) {
        ...
        comparisonResult === "Invalid" /* NodeComparisonResult.Invalid */
      ) {
