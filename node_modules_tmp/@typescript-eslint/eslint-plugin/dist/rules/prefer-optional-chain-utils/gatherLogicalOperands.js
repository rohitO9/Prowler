...
function isValidFalseBooleanCheckType(
  ...
  if (disallowFalseyLiteral) {
    ...
    if (
      types.some(
        (t) =>
          (0, ts_api_utils_1.isBooleanLiteralType)(t) &&
          t.intrinsicName === "false",
      ) ||
      types.some(
        (t) => (0, ts_api_utils_1.isStringLiteralType)(t) && t.value === "",
      ) ||
      types.some(
        (t) => (0, ts_api_utils_1.isNumberLiteralType)(t) && t.value === 0,
      ) ||
      types.some(
        (t) =>
          (0, ts_api_utils_1.isBigIntLiteralType)(t) &&
          t.value.base10Value === "0",
      )
    ) {
      return false;
    }
  ...
  for (const operand of operands) {
    ...
    switch (operand.type) {
      case utils_1.AST_NODE_TYPES.BinaryExpression: {
        ...
        if (
          comparedValue ===
          "UndefinedStringLiteral" /* ComparisonValueType.UndefinedStringLiteral */
        ) {
          if (
            comparedExpression.type ===
            ...
            if (argument.type === utils_1.AST_NODE_TYPES.Identifier) {
              const reference = sourceCode
                .getScope(argument)
                .references.find(
                  (ref) => ref.identifier.name === argument.name,
                );
        ...
        switch (operand.operator) {
          ...
          case "==":
            if (
              comparedValue === "Null" /* ComparisonValueType.Null */ ||
