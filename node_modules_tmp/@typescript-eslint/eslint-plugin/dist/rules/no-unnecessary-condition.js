...
exports.default = (0, util_1.createRule)({
  ...
  create(
    ...
    function checkIfBinaryExpressionIsNecessaryConditional(node) {
      ...
      const leftType = (0, util_1.getConstrainedTypeAtLocation)(
        services,
        node.left,
      );
      const rightType = (0, util_1.getConstrainedTypeAtLocation)(
        services,
        node.right,
      );
    ...
    function checkIfLoopIsNecessaryConditional(node) {
      ...
      if (
        ...
        tsutils.isTrueLiteralType(
          (0, util_1.getConstrainedTypeAtLocation)(services, node.test),
        )
      ) {
        return;
      }
      ...
      if (isArrayPredicateFunction(node) && node.arguments.length) {
        ...
        const returnTypes = tsutils
          .getCallSignaturesOfType(
            (0, util_1.getConstrainedTypeAtLocation)(services, callback),
          )
    ...
    function optionChainContainsOptionArrayIndex(node) {
      ...
      if (
        lhsNode.type === utils_1.AST_NODE_TYPES.MemberExpression ||
    ...
    function isNullablePropertyType(objType, propertyType) {
      ...
      return !!checker
        ...
        .find(
          (info) => (0, util_1.getTypeName)(checker, info.keyType) === typeName,
        );
    }
    ...
    function isMemberExpressionNullableOriginFromObject(node) {
      const prevType = (0, util_1.getConstrainedTypeAtLocation)(
        services,
        node.object,
      );
      const property = node.property;
      if (prevType.isUnion() && (0, util_1.isIdentifier)(property)) {
        const isOwnNullable = prevType.types.some((type) => {
          if (node.computed) {
            const propertyType = (0, util_1.getConstrainedTypeAtLocation)(
              services,
              node.property,
            );
      ...
      if (prevType.isUnion()) {
        const isOwnNullable = prevType.types.some((type) => {
          ...
          return signatures.some((sig) =>
            (0, util_1.isNullableType)(sig.getReturnType(), {
              allowUndefined: true,
            }),
          );
        });
