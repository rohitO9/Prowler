...
exports.default = (0, util_1.createRule)({
  ...
  create(context, [option]) {
    ...
    function checkMethodAccessibilityModifier(methodDefinition) {
      if (
        methodDefinition.key.type === utils_1.AST_NODE_TYPES.PrivateIdentifier
      ) {
        return;
      }
      ...
      const { name: methodName } = (0, util_1.getNameFromMember)(
        methodDefinition,
        context.sourceCode,
      );
      ...
      if (
        ...
        context.report({
          loc: (0, getMemberHeadLoc_1.getMemberHeadLoc)(
            context.sourceCode,
            methodDefinition,
          ),
    ...
    function findPublicKeyword(node) {
      ...
      for (let i = 0; i < tokens.length; i++) {
        ...
        if (
          token.type === utils_1.AST_TOKEN_TYPES.Keyword &&
          token.value === "public"
        ) {
    ...
    function getMissingAccessibilitySuggestions(node) {
      function fix(accessibility, fixer) {
        if (node.decorators.length) {
          ...
          const nextToken = (0, util_1.nullThrows)(
            context.sourceCode.getTokenAfter(lastDecorator),
            util_1.NullThrowsReasons.MissingToken("token", "last decorator"),
          );
    ...
    function checkPropertyAccessibilityModifier(propertyDefinition) {
      if (
        propertyDefinition.key.type === utils_1.AST_NODE_TYPES.PrivateIdentifier
      ) {
        return;
      }
      const nodeType = "class property";
      const { name: propertyName } = (0, util_1.getNameFromMember)(
        propertyDefinition,
        context.sourceCode,
      );
      if (
    ...
    function checkParameterPropertyAccessibilityModifier(node) {
      ...
      if (
        node.parameter.type !== utils_1.AST_NODE_TYPES.Identifier &&
      ...
      switch (paramPropCheck) {
        case "explicit": {
          if (!node.accessibility) {
            context.report({
              loc: (0, getMemberHeadLoc_1.getParameterPropertyHeadLoc)(
                context.sourceCode,
                node,
                nodeName,
              ),
