...
exports.default = (0, util_1.createRule)({
  ...
  create(context, [option]) {
    ...
    const emitDecoratorMetadata =
      (0, util_1.getParserServices)(context, true).emitDecoratorMetadata ??
      false;
    const experimentalDecorators =
      (0, util_1.getParserServices)(context, true).experimentalDecorators ??
      false;
    ...
    function getFixesNamedSpecifiers(
      ...
      if (subsetNamedSpecifiers.length === allNamedSpecifiers.length) {
        ...
        const openingBraceToken = (0, util_1.nullThrows)(
          context.sourceCode.getTokenBefore(
            subsetNamedSpecifiers[0],
            util_1.isOpeningBraceToken,
          ),
          util_1.NullThrowsReasons.MissingToken("{", node.type),
        );
        const commaToken = (0, util_1.nullThrows)(
          context.sourceCode.getTokenBefore(
            openingBraceToken,
            util_1.isCommaToken,
          ),
        ...
        const closingBraceToken = (0, util_1.nullThrows)(
    ...
    function getNamedSpecifierRanges(namedSpecifierGroup, allNamedSpecifiers) {
      ...
      const before = (0, util_1.nullThrows)(
        context.sourceCode.getTokenBefore(first),
        util_1.NullThrowsReasons.MissingToken("token", "first specifier"),
      );
      ...
      const after = (0, util_1.nullThrows)(
        context.sourceCode.getTokenAfter(last),
        util_1.NullThrowsReasons.MissingToken("token", "last specifier"),
      );
      ...
      const closingBraceToken = (0, util_1.nullThrows)(
        context.sourceCode.getFirstTokenBetween(
          (0, util_1.nullThrows)(
            ...
            util_1.NullThrowsReasons.MissingToken("token before", "import"),
          ),
          target.source,
          util_1.isClosingBraceToken,
        ),
        util_1.NullThrowsReasons.MissingToken("}", target.type),
      );
      const before = (0, util_1.nullThrows)(
        context.sourceCode.getTokenBefore(closingBraceToken),
        util_1.NullThrowsReasons.MissingToken("token before", "closing brace"),
      );
      if (
        !(0, util_1.isCommaToken)(before) &&
        !(0, util_1.isOpeningBraceToken)(before)
      ) {
    ...
    function* fixToTypeImportDeclaration(fixer, report, sourceImports) {
      ...
      if (typeNamedSpecifiers.length) {
        if (sourceImports.typeOnlyNamedImport) {
          ...
          if (fixStyle === "inline-type-imports") {
            ...
            yield fixer.insertTextBefore(
              node,
              `import type {${fixesNamedSpecifiers.typeNamedSpecifiersText}} from ${context.sourceCode.getText(node.source)};\n`,
            );
          }
        }
      }
      ...
      if (
        ...
        yield fixer.insertTextBefore(
          node,
          `import type ${context.sourceCode.getText(namespaceSpecifier)} from ${context.sourceCode.getText(node.source)};\n`,
        );
      }
      if (
        defaultSpecifier &&
        ...
        if (report.typeSpecifiers.length === node.specifiers.length) {
          const importToken = (0, util_1.nullThrows)(
            context.sourceCode.getFirstToken(node, util_1.isImportKeyword),
            util_1.NullThrowsReasons.MissingToken("import", node.type),
          );
          ...
          const commaToken = (0, util_1.nullThrows)(
            ...
            util_1.NullThrowsReasons.MissingToken(",", defaultSpecifier.type),
          );
          // import Type , {...} from 'foo'
          //        ^^^^^ pick
          ...
          yield fixer.insertTextBefore(
            node,
            `import type ${defaultText} from ${context.sourceCode.getText(node.source)};\n`,
          );
          const afterToken = (0, util_1.nullThrows)(
    ...
    function* fixRemoveTypeSpecifierFromImportDeclaration(fixer, node) {
      ...
      const typeToken = (0, util_1.nullThrows)(
        context.sourceCode.getFirstTokenBetween(
          importToken,
          node.specifiers[0]?.local ?? node.source,
          util_1.isTypeKeyword,
        ),
        util_1.NullThrowsReasons.MissingToken("type", node.type),
      );
      const afterToken = (0, util_1.nullThrows)(
    ...
    function* fixRemoveTypeSpecifierFromImportSpecifier(fixer, node) {
      ...
      //          ^^^^ remove
      const typeToken = (0, util_1.nullThrows)(
        context.sourceCode.getFirstToken(node, util_1.isTypeKeyword),
        util_1.NullThrowsReasons.MissingToken("type", node.type),
      );
      const afterToken = (0, util_1.nullThrows)(
        context.sourceCode.getTokenAfter(typeToken, { includeComments: true }),
