...
function getOpeningParenOfParams(node, sourceCode) {
  ...
  if (
    ...
    const argToken = utils_1.ESLintUtils.nullThrows(
      sourceCode.getFirstToken(node.params[0]),
      utils_1.ESLintUtils.NullThrowsReasons.MissingToken(
        "parameter",
        "arrow function",
      ),
    );
    const maybeParenToken = sourceCode.getTokenBefore(argToken);
    return maybeParenToken &&
      (0, astUtils_1.isOpeningParenToken)(maybeParenToken)
      ? maybeParenToken
      : argToken;
  }
  // Otherwise, returns paren.
  return node.id != null
    ? utils_1.ESLintUtils.nullThrows(
        sourceCode.getTokenAfter(node.id, astUtils_1.isOpeningParenToken),
        utils_1.ESLintUtils.NullThrowsReasons.MissingToken("id", "function"),
      )
    : utils_1.ESLintUtils.nullThrows(
        sourceCode.getFirstToken(node, astUtils_1.isOpeningParenToken),
        utils_1.ESLintUtils.NullThrowsReasons.MissingToken(
          "opening parenthesis",
          "function",
        ),
...
function getFunctionHeadLoc(node, sourceCode) {
  ...
  if (
    ...
    if (parent.decorators.length > 0) {
      ...
      const firstTokenAfterDecorator = utils_1.ESLintUtils.nullThrows(
    ...
    const arrowToken = utils_1.ESLintUtils.nullThrows(
      sourceCode.getTokenBefore(node.body, astUtils_1.isArrowToken),
      utils_1.ESLintUtils.NullThrowsReasons.MissingToken(
        "arrow token",
        "arrow function",
      ),
