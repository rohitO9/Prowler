...
function isTypedParent(parent, callee) {
  return (
    (0, astUtils_1.isTypeAssertion)(parent) ||
...
function doesImmediatelyReturnFunctionExpression({ node, returns }) {
  ...
  return returns.every(
    (node) => node.argument && utils_1.ASTUtils.isFunction(node.argument),
  );
}
/**
 * Checks if a function belongs to:
...
function returnsConstAssertionDirectly(node) {
  ...
  if ((0, astUtils_1.isTypeAssertion)(body)) {
    ...
    if (typeAnnotation.type === utils_1.AST_NODE_TYPES.TSTypeReference) {
      ...
      if (
        typeName.type === utils_1.AST_NODE_TYPES.Identifier &&
        typeName.name === "const"
      ) {
        return true;
      }
...
function isTypedFunctionExpression(node, options) {
  const parent = utils_1.ESLintUtils.nullThrows(
    node.parent,
    utils_1.ESLintUtils.NullThrowsReasons.MissingParent,
  );
...
function ancestorHasReturnType(node) {
  ...
  const isReturnStatement =
    ancestor.type === utils_1.AST_NODE_TYPES.ReturnStatement;
