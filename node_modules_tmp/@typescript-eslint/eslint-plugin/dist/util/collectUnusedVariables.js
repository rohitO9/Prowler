...
class UnusedVarsVisitor extends scope_manager_1.Visitor {
  ...
  ForInStatement(node) {
    ...
    if (node.left.type === utils_1.AST_NODE_TYPES.VariableDeclaration) {
      const variable = __classPrivateFieldGet(
        this,
        _UnusedVarsVisitor_scopeManager,
        "f",
      )
        .getDeclaredVariables(node.left)
        .at(0);
      if (!variable) {
        return;
      }
  ...
  Identifier(node) {
    const scope = this.getScope(node);
    if (
      scope.type === utils_1.TSESLint.Scope.ScopeType.function &&
      node.name === "this"
    ) {
...
function isExported(variable) {
  return variable.defs.some((definition) => {
    ...
    if (node.type === utils_1.AST_NODE_TYPES.VariableDeclarator) {
      ...
      definition.type === utils_1.TSESLint.Scope.DefinitionType.Parameter
    ) {
      return false;
    }
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
...
function isUsedVariable(variable) {
  ...
  function getFunctionDefinitions(variable) {
    ...
    variable.defs.forEach((def) => {
      ...
      // FunctionExpressions
      if (
        def.type === utils_1.TSESLint.Scope.DefinitionType.Variable &&
