...
async function pruneAsync() {
  ...
  if (args.length > 0) {
    throw new Error(
      `@rushstack/eslint-bulk: Unknown arguments: ${args.join(" ")}`,
    );
  }
  const normalizedCwd = process.cwd().replace(/\\/g, "/");
  const allFiles =
    await getAllFilesWithExistingSuppressionsForCwdAsync(normalizedCwd);
  if (allFiles.length > 0) {
    ...
    (0, bulk_suppressions_file_1.deleteBulkSuppressionsFileInEslintrcFolder)(
      normalizedCwd,
    );
  }
}
...
async function getAllFilesWithExistingSuppressionsForCwdAsync(normalizedCwd) {
  const { jsonObject: bulkSuppressionsConfigJson } = (0,
  ...
  await Promise.all(
    allFilesArray.map(async (filePath) => {
      try {
        await fs_1.default.promises.access(
          filePath,
  ...
  console.log(
    `Found ${allExistingFiles.length} files with existing suppressions.`,
  );
