...
function getEslintPath(packagePath) {
  ...
  try {
    const localEslintApiPath = require.resolve(
      constants_1.BULK_SUPPRESSIONS_CLI_ESLINT_PACKAGE_NAME,
      {
        paths: [packagePath],
      },
    );
    const localEslintPath = path_1.default.dirname(
      path_1.default.dirname(localEslintApiPath),
    );
    const { version: localEslintVersion } = require(
      `${localEslintPath}/package.json`,
    );
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          "@rushstack/eslint-bulk: eslint is not specified as a dependency in package.json.",
        );
      }
    } catch (e) {
      throw new Error(
