...
exports.extendVerifyFunction =
  exports.patchClass =
  exports.requireFromPathToLinterJS =
  exports.write =
  exports.prune =
  exports.shouldBulkSuppress =
    void 0;
...
const ESLINT_BULK_SUPPRESS_ENV_VAR_VALUE =
...
function getNodeName(node) {
  if (Guards.isClassDeclarationWithName(node)) {
    ...
    Guards.isNormalVariableDeclaratorWithAnonymousExpressionAssigned(node)
  ) {
    return node.id.name;
  } else if (
    Guards.isNormalObjectPropertyWithAnonymousExpressionAssigned(node)
  ) {
    return node.key.name;
  } else if (
    Guards.isNormalClassPropertyDefinitionWithAnonymousExpressionAssigned(node)
  ) {
    return node.key.name;
  } else if (
    Guards.isNormalAssignmentPatternWithAnonymousExpressionAssigned(node)
  ) {
...
function findEslintrcFolderPathForNormalizedFileAbsolutePath(
  normalizedFilePath,
) {
  const cachedFolderPathForFilePath =
    eslintrcPathByFileOrFolderPath.get(normalizedFilePath);
  if (cachedFolderPathForFilePath) {
    return cachedFolderPathForFilePath;
  }
  const normalizedFileFolderPath = normalizedFilePath.substring(
    0,
  ...
  findEslintrcFileLoop: for (
    let currentFolder = normalizedFileFolderPath;
    currentFolder; // 'something'.substring(0, -1) is ''
    currentFolder = currentFolder.substring(0, currentFolder.lastIndexOf("/"))
  ) {
    const cachedEslintrcFolderPath =
      eslintrcPathByFileOrFolderPath.get(currentFolder);
  ...
  if (eslintrcFolderPath) {
    ...
    throw new Error(
      `Cannot locate an ESLint configuration file for ${normalizedFilePath}`,
    );
  }
}
...
function shouldBulkSuppress(params) {
  ...
  const eslintrcDirectory = findEslintrcFolderPathForNormalizedFileAbsolutePath(
    normalizedFileAbsolutePath,
  );
  const fileRelativePath = normalizedFileAbsolutePath.substring(
    eslintrcDirectory.length + 1,
  );
  const scopeId = calculateScopeId(currentNode);
  const suppression = { file: fileRelativePath, scopeId, rule };
  const config = (0,
  bulk_suppressions_file_1.getSuppressionsConfigForEslintrcFolderPath)(
    eslintrcDirectory,
  );
  const serializedSuppression = (0,
  bulk_suppressions_file_1.serializeSuppression)(suppression);
  const currentNodeIsSuppressed = config.serializedSuppressions.has(
    serializedSuppression,
  );
  if (
    currentNodeIsSuppressed ||
    SUPPRESS_ALL_RULES ||
    (RULES_TO_SUPPRESS === null || RULES_TO_SUPPRESS === void 0
      ? void 0
      : RULES_TO_SUPPRESS.has(suppression.rule))
  ) {
...
function prune() {
  for (const [eslintrcFolderPath, suppressionsConfig] of (0,
  bulk_suppressions_file_1.getAllBulkSuppressionsConfigsByEslintrcFolderPath)()) {
    if (suppressionsConfig) {
      ...
      (0, bulk_suppressions_file_1.writeSuppressionsJsonToFile)(
        eslintrcFolderPath,
        newSuppressionsConfig,
      );
    }
  }
}
...
function patchClass(originalClass, patchedClass) {
  ...
  for (const [prop, descriptor] of Object.entries(
    Object.getOwnPropertyDescriptors(patchedClass.prototype),
  )) {
