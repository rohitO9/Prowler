...
exports.isModuleResolutionError =
  exports.ESLINT_MAJOR_VERSION =
  exports.Naming =
  exports.ModuleResolver =
  exports.configArrayFactory =
  exports.eslintFolder =
  exports.eslintPackageVersion =
    void 0;
...
for (let currentModule = module; ; ) {
  if (!eslintrcBundlePath) {
    if (currentModule.filename.endsWith("eslintrc.cjs")) {
      ...
      try {
        const eslintrcFolderPath = path_1.default.dirname(
        ...
        const resolvedEslintrcBundlePath = path_1.default.join(
          eslintrcFolderPath,
          "dist/eslintrc.cjs",
        );
    ...
    try {
      const eslintCandidateFolder = path_1.default.dirname(
      ...
      // and not some other spurious dependency.
      if (
        currentModule.filename.startsWith(
          eslintCandidateFolder + path_1.default.sep,
        )
      ) {
...
if (!eslintFolder) {
  ...
  for (let currentModule = module; ; ) {
    if (!configArrayFactoryPath) {
      ...
      try {
        const eslintrcFolder = path_1.default.dirname(
          require.resolve("@eslint/eslintrc/package.json", {
            paths: [currentModule.path],
          }),
        );
        const resolvedConfigArrayFactoryPath = path_1.default.join(
          eslintrcFolder,
...
if (!eslintFolder) {
  ...
  for (let currentModule = module; ; ) {
    ...
    if (
      ...
      exports.eslintFolder = eslintFolder = path_1.default.join(
        path_1.default.dirname(currentModule.filename),
        "../..",
      );
      ...
      let eslintrcFolder;
      try {
        eslintrcFolder = path_1.default.dirname(
          require.resolve("@eslint/eslintrc/package.json", {
            paths: [currentModule.path],
          }),
        );
...
if (isNaN(ESLINT_MAJOR_VERSION)) {
  throw new Error(
    `Unable to parse ESLint version "${exports.eslintPackageVersion}" in file "${eslintPackageJsonPath}"`,
  );
}
...
if (ESLINT_MAJOR_VERSION >= 8) {
  exports.configArrayFactory = configArrayFactory =
    require(eslintrcBundlePath).Legacy.ConfigArrayFactory;
} else {
  exports.configArrayFactory = configArrayFactory = require(
