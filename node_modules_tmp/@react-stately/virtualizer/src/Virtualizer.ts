...
export class Virtualizer<T extends object, V, W> {
  ...
  getReusableView(layoutInfo: LayoutInfo): ReusableView<T, V> {
    ...
    let view =
      reusable.length > 0 ? reusable.pop() : new ReusableView<T, V>(this);
   ...
   */
  relayoutNow(
    context: InvalidationContext<T, V> = this._invalidationContext || {},
  ) {
  ...
  private _restoreScrollAnchor(
    scrollAnchor: ScrollAnchor | null,
    context: InvalidationContext<T, V>,
  ) {
  ...
  updateSubviews(forceUpdate = false) {
    ...
    if (forceUpdate) {
      ...
      ({ toAdd, toRemove, toUpdate } = difference(
        currentlyVisible,
        visibleLayoutInfos,
      ));
  ...
  private _flushVisibleViews() {
    ...
    for (let view of this._children) {
      if (
        view.layoutInfo?.parentKey != null &&
    ...
    this.relayoutNow({
      ...
      afterAnimation: () => {
        ...
        if (transaction.toRemove.size > 0 || transaction.removed.size > 0) {
          for (let view of concatIterators(
