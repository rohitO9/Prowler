...
export class SelectionManager implements MultipleSelectionManager {
  ...
  get firstSelectedKey(): Key | null {
    ...
    for (let key of this.state.selectedKeys) {
      ...
      if (
        !first ||
        (item && compareNodeOrder(this.collection, item, first) < 0)
      ) {
        first = item;
      }
    }
  ...
  get lastSelectedKey(): Key | null {
    ...
    for (let key of this.state.selectedKeys) {
      ...
      if (
        !last ||
        (item && compareNodeOrder(this.collection, item, last) > 0)
      ) {
        last = item;
      }
    }
  ...
  extendSelection(toKey: Key) {
    ...
    if (this.state.selectedKeys === "all") {
      ...
      for (let key of this.getKeyRange(
        anchorKey,
        selectedKeys.currentKey ?? mappedToKey,
      )) {
  ...
  isDisabled(key: Key) {
    return (
      ...
      (this.state.disabledKeys.has(key) ||
        !!this.collection.getItem(key)?.props?.isDisabled)
    );
  }
